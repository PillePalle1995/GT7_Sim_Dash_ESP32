...
class GT7_UDP_Parser {
    public:
        ...
        uint8_t getcurrentThrottle(void);
        uint8_t getcurrentBrake(void);
        int32_t getlastLaptime(void);
        int32_t getbestLaptime(void);
        int32_t getcurrentLaptime(void);
        int16_t getminAlertRPM(void);
        int16_t getmaxAlertRPM(void);
        float getcurrenttyreTemp(int index);
        uint8_t getcurrentBrakeFiltered(void);
        uint8_t getcurrentThrottleFiltered(void);
        float getcurrentFuelLevel(void);
        float getcurrentOilTemp(void);
        float getcurrentOilPressure(void);
        float getcurrentBoost(void);
        float getcurrentCoolantTemp(void);
        int16_t getcurrentLapCount(void);
        // Packet readData();
    private: 
        // WiFiUDP Udp;
        // IPAddress remoteIP;
